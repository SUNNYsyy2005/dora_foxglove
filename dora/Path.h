...
class Path{
    public:
        ...
        static void sendPath(const Path& scan, boost::asio::io_context& io_context, boost::asio::ip::tcp::socket &socket) {
            try {
                boost::asio::ip::tcp::resolver resolver(io_context);
                boost::asio::connect(socket, resolver.resolve("127.0.0.1", std::to_string(8762)));
